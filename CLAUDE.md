...
## 🔍 코드 리뷰 스코핑 (MR/PR 리뷰 시)

MR·PR 리뷰를 요청받으면 반드시 **해당 MR 의 diff 범위(GitLab "변경 사항/Changes" 탭에
보이는 바로 그 범위)만** 검토한다.

- ❌ `develop`·base 기준 **누적 diff(수십 커밋·수십 파일)를 통째로 끌어오지 말 것.**
  실제 MR 변경분이 아닌 것까지 섞여 리뷰 스코프가 어긋난다.
- ✅ 리뷰 시작 전에 **검토 대상 diff 범위와 변경 파일 목록을 먼저 제시**해 사용자와
  스코프를 맞춘다.
- 리뷰에서 특정 helper·심볼을 언급할 때는 **그 심볼이 실제로 그 파일에 존재하는지
  먼저 확인**한 뒤 코멘트한다. (위치 추측 금지)

애매하면 리뷰 전에 대상 MR 번호·범위를 사용자에게 확인한다.
...
- **`!` 가 들어간 문자열(비밀번호·JSON 페이로드 등)을 zsh 인라인 명령에 그대로 쓰지
  않는다.** zsh 히스토리 확장이 `!` 를 건드려 값이 깨지고, 로그인 페이로드라면
  엉뚱한 401 이 떠 디버깅이 헛돈다.
  - 예: 테스트 매니저 비밀번호처럼 끝에 `!` 가 붙는 값(`hancom123!`).
  - ✅ 해결: 페이로드를 **heredoc·스크립트 파일·임시 파일로 작성**해 전달하거나,
    값을 **작은따옴표(`'...'`)로 감싸** 히스토리 확장을 끈다. (`curl -d @payload.json` 등)
